...
export default {
  ...
  methods: {
    initialize() {
      ...
      this.$http.get('http://localhost:8080/api/version')
          .then(result => {
    ...
    editItem(item) {
      this.editedIndex = this.versoes.indexOf(item)
    ...
    deleteItemConfirm() {
      this.$http.delete(`http://localhost:8080/api/version/${this.editedItem.id}`).then(result => {
        this.success = true
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        this.$http.put(`http://localhost:8080/api/version/${this.editedItem.id}`, {
        ...
        this.$http.post('http://localhost:8080/api/version', {
          descricao: this.editedItem.descricao
        }).then(result => {
          this.success = true
          this.messageSucesso = result.data.msg;
